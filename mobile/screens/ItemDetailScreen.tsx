...
// View full item details + Edit and Delete actions
// ============================================================
...
export default function ItemDetailScreen() {
  ...
  // Refetch when screen comes back into focus (e.g. after edit)
  useFocusEffect(
    useCallback(() => {
      getItemById(itemId).then(setItem).catch(() => nav.goBack())
    }, [itemId])
  )
  ...
  return (
    <ScrollView style={s.container}>
      ...
      <View style={s.header}>
        ...
        <TouchableOpacity onPress={() => nav.navigate('EditItem', { itemId })}>
          <Text style={s.edit}>Edit ✏️</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={s.card}>
        ...
        <Row label="Storage"         value={`${item.storage_locations?.icon ?? ''} ${item.storage_locations?.name ?? '—'}`} />
...
const s = StyleSheet.create({
  ...
  edit: { fontSize: 15, color: '#4A42B0', fontWeight: '600' },
  hero: {
  ...
  card: {
    ...
    borderRadius: 14, borderWidth: 0.5, borderColor: '#E4E4E4', marginBottom: 16,
  },
  row: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
  ...
  deleteBtn: {
    marginHorizontal: 16, marginBottom: 48, paddingVertical: 16,
